class OverworldMap {
  constructor(config) {
    this.overworld = null;
    this.configObjects = config.configObjects; // Содержание конфигурации
    this.gameObjects = {}; // Сначала пустой, сюда будут добавляться экземпляры живых объектов на карте
    this.cutsceneSpaces = config.cutsceneSpaces || {}; 
    this.walls = config.walls || {};  // Сцены на картах

    this.lowerImage = new Image(); //Нижняя часть изображения (карты). Реализованно так, чтобы слои работали правильно
    this.lowerImage.src = config.lowerSrc; // Ссылка на нижнюю часть карты

    this.upperImage = new Image(); // Верхняя часть карты
    this.upperImage.src = config.upperSrc;

    this.isCutscenePlaying = false; // Используется для блокировки действий игрока и других объектов при катсцене
    this.isPaused = false;  //Действует также, как и isCutscenePlaying, но не для катсцены, а для паузы
  }

  drawLowerImage(ctx, cameraPerson) { // Рендеринг верхней части карты
    ctx.drawImage(
      this.lowerImage, 
  ...
  drawUpperImage(ctx, cameraPerson) { // Рендеринг нижней части карты
    ctx.drawImage(
      this.upperImage, 
      utils.withGrid(10.5) - cameraPerson.x, 
      utils.withGrid(6) - cameraPerson.y
  ...
  isSpaceTaken(currentX, currentY, direction) { //Проверка пустое ли пространство перед игроком...
  ...
  }//Используется для "адекватной" коллизии

  mountObjects() {//Установка объектов на карту
  ...
  async startCutscene(events) { //Ассинронный запуск катсцены
    this.isCutscenePlaying = true;

    for (let i=0; i<events.length; i++) {//Запуск всех катсцен по очередно (не катсцен, а элементов катсцены(например текст который говорят npc))
  ...
  checkForActionCutscene() {// Проверка, запущена ли катсцена
...
window.OverworldMaps = { // Фактическое создание всех элеметов на карте (а также самой карты)
  DemoRoom: {
    id: "DemoRoom",
